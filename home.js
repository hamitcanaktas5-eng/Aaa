...
function buildMatchCard(m) {
  ...
  return `<div class="match-card${m.status === 'live' ? ' live' : ''}" data-mid="${m.id}">
    ...
    <div class="team home">
      ...
      <button class="team-fav${favHome?' active':''}" data-tid="${m.home.id}" data-tname="${m.home.name}" data-tshort="${m.home.short||m.home.name.substring(0,3).toUpperCase()}" data-tcolor="${m.home.color}" data-tcolor2="${m.home.color2||''}" data-tleague="${m.leagueName}">
    ...
    <!-- AWAY -->
    <div class="team away">
      <button class="team-fav${favAway?' active':''}" data-tid="${m.away.id}" data-tname="${m.away.name}" data-tshort="${m.away.short||m.away.name.substring(0,3).toUpperCase()}" data-tcolor="${m.away.color}" data-tcolor2="${m.away.color2||''}" data-tleague="${m.leagueName}">
        <svg viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
      </button>
      <div class="team-name-wrap">
    ...
    <!-- NOTIF BELL (hover) -->
    <button class="match-notif${favMatch?' active':''}" data-mid="${m.id}">
...
function bindEvents() {
  ...
  document.querySelectorAll('.match-card').forEach(card => {
    card.addEventListener('click', e => {
      if (e.target.closest('.team-fav') || e.target.closest('.match-notif') || e.target.closest('.team-name')) return;
  ...
  document.querySelectorAll('.match-notif').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      btn.classList.toggle('active', added);
      showToast(added?'🔔':'🔕', added?'Maç favorilere eklendi, şeritte görünecek':'Maç favorilerden çıkarıldı', '', 'neutral');
      renderFavStrip();
    });
  });
}
...
document.getElementById('search-input').addEventListener('input', e => {
  ...
  if (matchRes.length) {
    html += `<div class="search-section-label">Maçlar</div>`;
    html += matchRes.slice(0,5).map(m => {
      const cls = m.status === 'live' ? 'sr-live' : m.status === 'finished' ? 'sr-finished' : 'sr-upcoming';
      const txt = m.status === 'live' ? `● ${m.minute}'` : m.status === 'finished' ? 'MS' : m.time;
      return `<div class="search-result-item" onclick="goTo('match.html?id=${m.id}')">
        <span style="font-size:18px">${m.leagueFlag}</span>
        <div class="sr-info">
          <div class="sr-name">${m.home.name} — ${m.away.name}</div>
          <div class="sr-sub">${m.leagueName}</div>
        </div>
        <span class="sr-badge ${cls}">${txt}</span>
      </div>`;
    }).join('');
  }
